...
| 线性/径向/角向渐变、stops、pattern、2.5D transform | [绘制记录、图案、变换与能力](./paint) | `LinearGradient`、`RadialGradient`、`SectorAngularGradient`、`StripePattern`、`WornPattern`、`PointerTilt` |
...
- 查具体签名、字段、默认值和示例时，再进入对应任务页。
- 改 public API 时，同步更新概念页和详细任务页；不要只改 demo 或注释。
